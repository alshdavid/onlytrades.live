...
impl CTraderConnection {
  pub async fn connect(options: CTraderConnectionOptions) -> anyhow::Result<Self> {
    ...
    tokio::task::spawn(CTraderConnection::task_heart_beat(Arc::clone(&writer)));
    tokio::task::spawn(CTraderConnection::task_read(reader, Arc::clone(&listeners)));
...
#[derive(Debug, Error)]
pub enum ProtoError {
  #[error("The protocol message contains no body")]
  NoBody,
...
impl ProtoMessageParse for ProtoMessage {
  ...
  fn try_payload_type(&self) -> std::result::Result<ProtoOaPayloadType, ProtoError> {
    let payload_i32 = match i32::try_from(self.payload_type) {
      Ok(val) => val,
      Err(_) => return Err(ProtoError::PayloadParseError(self.payload_type)),
    };

    match ProtoOaPayloadType::try_from(payload_i32) {
      Ok(payload_type) => Ok(payload_type),
      Err(_) => Err(ProtoError::UnknownPayloadType(payload_i32)),
    }
  }
}
