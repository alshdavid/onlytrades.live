...
use crate::generic_process::GenericProcess;
use crate::generic_process::GenericProcessOptions;

pub struct IpcTcpInstance {
  pub id: Uuid,
...
impl std::fmt::Debug for IpcTcpInstance {
  fn fmt(
    &self,
    f: &mut std::fmt::Formatter<'_>,
  ) -> std::fmt::Result {
    f.debug_struct("IpcTcpInstance")
      .field("id", &self.id)
      .finish()
  }
}
...
impl IpcTcpInstance {
  pub async fn new(mut options: GenericProcessOptions) -> anyhow::Result<Self> {
    ...
    let hostname = match options.sandbox {
      SandboxType::None { command: _ } => "127.0.0.1".to_string(),
      SandboxType::Podman {
        image: _,
        command: _,
    ...
    let child = GenericProcess::launch(options).await?;
